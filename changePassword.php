...
if(isset($_POST['submit'])){
  if($data['cek_pass'] == 1){
    ...
    $password = md5($pass);
      $id = $data['id'];
    $koneksi->query("UPDATE akun SET password='$password' WHERE id =$id");
    header("Location: login.php");
    }
				...
				<div class="col-12 text-center align-self-center">
					<div class="section pb-5 pt-sm-2 text-center">
						<div class="card-3d-wrap mx-auto">
							<div class="card-3d-wrapper"  id="card-confir" >
								<div class="card-front" style="position: relative;">
									<div class="">
										<div class="subconfir section text-center">
											<h4 class="mb-4 pb-3">Reset Password</h4>
                      <form method = "POST" >
                        <div class="form-group mt-2">
                            <input type="password" name="password" class="form-style" placeholder="New Password" id="logpass" autocomplete="off" required>
                            <i class="input-icon"><span class="material-symbols-outlined">
                              lock
                              </span></i>
                          </div>
                          <div class="form-group mt-2">
                            <input type="password" name="confirmpass" class="form-style" placeholder="Confirm Password" id="logpass" autocomplete="password" required>
                            <i class="input-icon"><span class="material-symbols-outlined">
                              lock
                              </span></i>
                          </div>
                        <button name = 'submit' type = 'submit' class="btn mt-4">submit</button>
                      </form>						
			      				</div>
			      			</div>
			      		</div>
